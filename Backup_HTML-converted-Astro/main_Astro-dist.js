...
if (themeToggleBtn) {
    themeToggleBtn.addEventListener('click', () => {
        ...
        if (themeToggle) themeToggle.innerHTML = document.body.classList.contains('dark-theme') ? '☀️' : '🌙';
...
window.openPost = async function(filename) {
    ...
    try {
        ...
        // Strip YAML frontmatter added for CMS
...
// ==========================================
// 9.5 DYNAMIC JSON-BASED BLOG FEED (REMOVED - NOW STATICALLY GENERATED VIA ASTRO)
// ==========================================
async function initDynamicBlogFeed() {
    const grid = document.querySelector('.blog-grid');
    if (!grid) return;

    // Blog cards are now statically generated by Astro at build time.
    // We only need to initialize sorting, prefetching, and URL parameters.
...
// ==========================================
// 10. CLIENT-SIDE BLOG SORTING & FILTERING
// ==========================================
function initBlogSorting() {
    ...
    function applyFilters() {
        ...
        cards.forEach(card => {
            ...
            const categoryAttr = card.dataset.category || 'Uncategorized';
            
            const matchesSearch = !query || dateText.includes(query) || titleText.includes(query) || descText.includes(query) || tagsText.includes(query);
    ...
    if (filterBtns) {
        filterBtns.forEach(btn => {
            btn.addEventListener('click', (e) => {
                filterBtns.forEach(b => b.classList.remove('active', 'custom-cursor-active')); // remove old styles
...
function hidePreloader() {
    ...
    if (preloader && !preloader.classList.contains('fade-out')) {
        ...
        }, 500); // Wait for the CSS fade-out transition
    }
}
